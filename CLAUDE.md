...
This runs on a Raspberry Pi under Linux and depends on it: `aplay` for TTS playback, BlueZ over D-Bus for Bluetooth and controller battery, `adb` for the phone, ALSA/pygame for audio. Most modules cannot be imported on a Windows dev machine — expect to reason about them statically or test on the Pi. `requirements.txt` has drifted before; treat imports, not that file, as the dependency list. The realtime voice pathway needs `websockets`, which nothing else in the project uses.
